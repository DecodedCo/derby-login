  ...
  function parseError(err, redirectData) {
    ...
    if (!err) {
      if (redirectData && redirectData.registrationData){
        return { success: true, url: options.passport.successRedirect + '/course-project/' + redirectData.registrationData.courseId + '/' + redirectData.registrationData.courseName };
      } else {
